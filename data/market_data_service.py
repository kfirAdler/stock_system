...
class MarketDataService:
    """Coordinates market data fetch, validation, and Supabase cache updates."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.supabase_cache is None or not self.supabase_cache.enabled:
            raise RuntimeError("Supabase cache must be configured and enabled")
    ...
    def get_histories(
        ...
        if worker_count == 1:
            for idx, ticker in enumerate(tickers, start=1):
                history = self.get_history(ticker=ticker, use_cache=use_cache, allow_remote_fetch=allow_remote_fetch)
        ...
        futures = {
            executor.submit(self.get_history, ticker=ticker, use_cache=use_cache, allow_remote_fetch=allow_remote_fetch): ticker
    ...
    def _fetch_and_cache_full(self, ticker: str) -> pd.DataFrame:
